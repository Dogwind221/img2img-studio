...
description: Fallback behavior when direct-api lacks OpenAI API credentials but Codex/native image generation is available
---

# Codex Image2 Fallback

When using `direct-api` with `--provider openai --model gpt-image-2`, the CLI can fail with:
...
1. Try `direct-api` when provider credentials are available.
...
3. Prefer a Codex/native raster backend in this order:
   ...
   - `direct-api --provider codex-cli` (preferred — wraps the bundled `scripts/codex-imagegen/main.ts`; the underlying repo-level package lives at `packages/img2img-codex/src/main.ts` for standalone callers), if `codex` CLI is installed/logged in.
...
Avoid implying that `direct-api --provider openai` can use Codex OAuth without a dedicated provider implementation.
